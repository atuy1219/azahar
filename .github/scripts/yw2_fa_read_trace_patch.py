...
def patch_fs_file() -> None:
    ...
    if "debug.azahar.yw2_fs_read_trace" not in text:
        ...
        helper = (
            ...
            "}\n\n"
            + TARGETS_CPP +
            "u32 YW2ReadStackWord(VAddr addr) {\n"
            "    if (addr == 0) {\n"
            "        return 0xFFFFFFFF;\n"
            "    }\n"
            ...
            "    const u32 s00 = YW2ReadStackWord(sp + 0x00);\n"
            "    const u32 s04 = YW2ReadStackWord(sp + 0x04);\n"
            "    const u32 s08 = YW2ReadStackWord(sp + 0x08);\n"
            "    const u32 s0C = YW2ReadStackWord(sp + 0x0C);\n"
            "    const u32 s10 = YW2ReadStackWord(sp + 0x10);\n"
            "    const u32 s14 = YW2ReadStackWord(sp + 0x14);\n"
            "    const u32 s18 = YW2ReadStackWord(sp + 0x18);\n"
            "    const u32 s1C = YW2ReadStackWord(sp + 0x1C);\n\n"
            "    for (const auto& target : YW2_READ_TARGETS) {\n"
            "        if (!YW2RangesOverlap(offset, length, target.offset, target.size)) {\n"
            "            continue;\n"
            "        }\n\n"
...
print("Patched YW2 FS read caller register/stack trace")
